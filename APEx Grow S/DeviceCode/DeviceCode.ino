...
void setup() {
  Wire.begin();
  Serial.begin(9600);
  pinMode(MainLED, OUTPUT), pinMode(StirMotor, OUTPUT), pinMode(CycleMotor, OUTPUT), pinMode(CMotorIn1, OUTPUT), pinMode(CMotorIn2, OUTPUT), pinMode(LangerIN, OUTPUT), pinMode(LangerOUT, OUTPUT), pinMode(LED, OUTPUT), pinMode(MMPin,OUTPUT);
  analogWrite(MainLED, 0), analogWrite(StirMotor, 0), analogWrite(LangerIN, 0), analogWrite(LangerOUT, 0), analogWrite(CycleMotor, 0), analogWrite(MMPin, 0), digitalWrite(LEDSig, LOW); /////
